...
<head>
  ...
  <title>Piedra Admin Panel </title>
  <!-- plugins:css -->
      ...
      <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-start">
        ...
        <div>
          <a class="navbar-brand brand-logo" href="{{ route('admin.index') }}">
            <img src="{{  (!empty($admin->image)? url('upload/admin_images/'.$admin->image):url('upload/admin_images/icon-admin.png')  )}}" alt="logo" />
          </a>
      ...
      <div class="navbar-menu-wrapper d-flex align-items-top"> 
        <ul class="navbar-nav">
          <li class="nav-item font-weight-semibold d-none d-lg-block ms-0">
            <h1 class="welcome-text">Welcome to Piedra's Admin Panel ,<span class="text-black fw-bold"> {{  $admin->name  }}  </span></h1>
          </li>
        </ul>
        <ul class="navbar-nav ms-auto">
          ...
          <li class="nav-item dropdown d-none d-lg-block user-dropdown">
            <a class="nav-link" id="UserDropdown" href="#" data-bs-toggle="dropdown" aria-expanded="false">
              <img class="img-xs rounded-circle" src="{{  (!empty($admin->image)? url('upload/admin_images/'.$admin->image):url('upload/admin_images/icon-admin.png')  )}}" > </a>


            <div class="dropdown-menu dropdown-menu-right navbar-dropdown" aria-labelledby="UserDropdown">
              <div class="dropdown-header text-center">
                <img class="img-md rounded-circle"  src="{{  (!empty($admin->image)? url('upload/admin_images/'.$admin->image):url('upload/admin_images/icon-admin.png')  )}}" width="20px" height="auto" alt="Profile image">
